...
export default async function FollowersPage({ params: { username } }: { params: { username: string } }) {
  ...
  const { data: rows, error: rowsErr } = await sb.rpc('get_followers', {
    p_user_id: profileId,
    p_limit: 200,
    p_offset: 0,
  });
  ...
  const ids: string[] = (rows ?? [])
    .map((r: any) => r.follower_id as string)
    .filter((v: string | undefined): v is string => !!v);

  // Fetch follower profiles and rebuild list in RPC order
  let people: any[] = [];
  if (ids.length) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 space-y-6">
      ...
      <div className="rounded-2xl border border-gray-800 bg-gray-900/60 p-5">
        ...
        <div className="mt-4 text-gray-400 text-sm">Followers of {name}</div>
      </div>
